...
- `sdk/go/`：Go 公共基础能力、fail-close 网关、异步日志客户端和 namespace 绑定对象存储 SDK。
...
详细说明见[SDK 内容](docs/sdk-content.md)，语言 SDK 的使用方法见[SDK 接入教程](docs/sdk/README.md)，Kafka 接入见[Go Kafka SDK](docs/sdk/go/kafka.md)，网关接入见[Go 网关 SDK](docs/sdk/go/gateway.md)，对象存储服务部署见[storage-service 部署与权限边界](docs/storage-service.md)，平台服务与业务项目的整体接入步骤见[接入 SDK](docs/sdk-integration.md)，版本与不可变制品规则见[发布与版本引用](docs/release.md)。
