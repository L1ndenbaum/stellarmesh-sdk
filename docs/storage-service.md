...
测试环境可以按固定版本引用 `ghcr.io/l1ndenbaum/stellarmesh-sdk/storage-service:0.2.0`；生产环境必须把已经验证的版本解析为 digest，并引用 `ghcr.io/l1ndenbaum/stellarmesh-sdk/storage-service@sha256:<摘要>`。本次 `0.2.0` 由统一镜像发布列车生成，Storage 运行时代码没有协议变化。镜像为公开制品，可以匿名拉取，不需要把 GitHub 凭据写入 Compose 或镜像。
