...
当前公开 Go/Python Logging 为 `0.4.0`，Python 主干准备 `0.5.0` 双格式支持。应用在本地选择 pretty、采集时选择 JSON；SDK 不读取环境配置。清洗行为见[共享约定](../contracts/logging/sanitization.md)；原生等级差异（如 Go `WARN` 与 Python `WARNING`）需要项目 Collector 显式映射。
