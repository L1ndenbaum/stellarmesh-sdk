...
- `stellarmesh-logging` 和 `stellarmesh-storage` Python 包从同一 commit 分别构建，并发布到团队管理的内部 Python registry；
...
- `ghcr.io/<组织>/stellarmesh-storage-service`；
- `ghcr.io/<组织>/stellarmesh-logging-clickhouse-sink`；
- `ghcr.io/<组织>/stellarmesh-logging-clickhouse-migrate`。

工作流同时生成版本 tag、提交短 SHA tag、构建来源证明和 SBOM。业务仓库可以用版本 tag 完成测试，但生产清单必须把验证后的镜像解析为 digest，并以 `image@sha256:...` 形式交给 `server-infrastructure`。四个镜像的 digest 应记录在同一份发布清单中，迁移镜像仍只作为一次性任务运行。
...
Python 包名分别为 `stellarmesh-logging` 和 `stellarmesh-storage`。二者独立发布、独立安装，但版本必须指向同一验证 commit。本仓库的镜像发布工作流不会假定团队使用哪一种 Python registry，也不会持有 registry 凭据。发布者应在已经通过持续验证的 commit 上分别构建 wheel 和源码包，再由受保护的发布任务上传：
...
业务仓库按需从内部 registry 固定安装 `stellarmesh-logging==X.Y.Z` 或 `stellarmesh-storage==X.Y.Z`，不要在生产镜像中直接安装可变分支。本次仓库改造只准备发布结构，不创建 tag、不推送镜像，也不上传 Python 包。

## 发布后验证

发布完成后至少确认：

1. Go module tag、两个 Python 包和四个镜像都对应同一 commit；
