...
已发布 `sdk/go/gateway/v0.5.0`。默认忽略并移除客户端传入的请求 ID，重新生成；受控入口可通过 `WithRequestID(RequestIDConfig{TrustIncoming: true})` 沿用单个合法值。重复或非法值重新生成，生成失败仍返回 `503`。这是默认行为变化，升级前请阅读[迁移说明](sdk/go/gateway-migration.md#从-v040-升级到-v050)。其他组件版本不变；源码和公开 Module 校验信息见[发布记录](releases/history.md#gateway-v050-请求-id-信任调整)。
