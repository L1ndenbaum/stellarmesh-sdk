...
前端包位于 `sdk/frontend/`，包名为 `@stellarmesh/sdk`，本文对应 `0.3.0`。它封装 Axios HTTP 传输、Fetch SSE、响应处理、鉴权刷新、重试和对象字节传输，不包含项目 DTO、分页兼容、上传会话或页面状态。

安装命令为 `npm install @stellarmesh/sdk@0.3.0`。包采用 MIT 许可证；发布状态、安装与制品验证步骤见[发布说明](../../release.md#前端-http-sdk-首次-npm-发布)。
