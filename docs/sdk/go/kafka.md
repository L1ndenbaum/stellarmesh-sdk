...
它只依赖 `kafka-go` 及其压缩、SCRAM 传递依赖，不会因为项目只需要 Kafka 而引入父 SDK、Object Storage、Gateway、JWT 或 Redis。本文安装示例适用于 `v0.1.0`，发布状态见[发布矩阵](../../release.md#当前制品矩阵)，要求 Go 1.24 及以上版本。
...
`sdk/go/v0.1.1` 仍包含旧的 `mq/kafka` package。它不能与新的 Kafka Module 同时使用，否则同一个 import path 会同时由两个 Module 提供，Go 可能报告 `ambiguous import`。已有 tag 保持不可变；新项目应使用已经移除 Kafka、Logging、Gateway 和 Object Storage package 的父 SDK `v0.5.0`，而不是为任一 Module添加长期 `replace`。
...
7. 如果项目还依赖父 SDK或 Logging，`go.mod` 同时固定父 `v0.5.0`、Logging `v0.4.0` 和 Kafka `v0.1.0`，且没有 `ambiguous import`。
