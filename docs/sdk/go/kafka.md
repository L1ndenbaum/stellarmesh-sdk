...
`sdk/go/v0.1.1` 仍包含旧的 `mq/kafka` package。它不能与新的 Kafka Module 同时使用，否则同一个 import path 会同时由两个 Module 提供，Go 可能报告 `ambiguous import`。已有 tag 保持不可变；新项目应使用已经移除 Kafka、Logging 和 Gateway package 的父 SDK `v0.4.0`，而不是为任一 Module添加长期 `replace`。
...
7. 如果项目还依赖父 SDK或 Logging，`go.mod` 同时固定父 `v0.4.0`、Logging `v0.1.0` 和 Kafka `v0.1.0`，且没有 `ambiguous import`。
