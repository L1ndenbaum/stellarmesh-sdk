...
它只依赖 `kafka-go` 及其压缩、SCRAM 传递依赖，不会因为项目只需要 Kafka 而引入父 SDK、Object Storage、Gateway、JWT 或 Redis。当前公开版本为 `v0.1.0`，要求 Go 1.24 及以上版本。
...
7. 如果项目还依赖父 SDK或 Logging，`go.mod` 同时固定父 `v0.5.0`、Logging `v0.1.0` 和 Kafka `v0.1.0`，且没有 `ambiguous import`。
