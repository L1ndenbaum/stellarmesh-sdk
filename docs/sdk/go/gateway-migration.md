...
`v0.5.1` 修复内置反向代理的请求 ID 一致性，不改变公开 API 和 `TrustIncoming` 默认值。代理在逐跳头清理后，从上下文恢复入口选定的 ID，防止 `Connection` 指名该请求头时丢失；复制后端响应头前移除同名字段，避免客户端收到重复值。默认 `X-Request-ID` 与自定义 `RequestIDConfig.Header` 均适用，整个过程不重新生成 ID。

使用 `WithUpstreams` 的项目升级 Module 即可生效；自定义 `WithUpstreamResolver` 的处理器仍需自行保证相同约定。`v0.5.0` 的默认入站信任变化见下文，详细行为见[请求 ID 配置](gateway-configuration.md#请求-id)。
