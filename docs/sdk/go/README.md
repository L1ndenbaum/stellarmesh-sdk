...
- [Go 网关 SDK](gateway.md)：提供声明式 Gateway、JWT 认证、Redis 限流与标准库访问日志；
- [Redis Session 认证](sessionauth.md)：要求 Gateway `v0.4.0` 或更新版本，提供 Cookie 凭证接入、项目隔离、每用户会话上限和踢下线；
- [Go Logging SDK](logging.md)：提供零第三方依赖的 `slog.Handler` 字段清洗装饰器；
...
[完整示例](../../../sdk/go/http/jsonbody/example_test.go)通过公开 import 使用组件。本例使用本地请求，不监听端口。Decode 负责关闭请求体，响应协议由调用方定义。
